...
function crearTarjetas(filosofos) {
    filosofos.forEach((filosofo) => {
        ...
        let textoPais = document.createElement('span');
        textoPais.classList.add('pais');
        textoPais.innerHTML = ` ${filosofo.pais.nombre}`;
        infoPais.append(bandera, textoPais);
        ...
        infoCorriente.innerHTML = `<span>Corriente: </span><span class="corriente">${filosofo.corriente}</span>`;
        ...
        infoArma.innerHTML = `<span>Arma: </span><span class="arma">${filosofo.arma}</span>`;
        ...
        for (let habilidad of filosofo.habilidades) {
            ...
            icono.alt = `Icono de ${habilidad.habilidad}`;
            skillBox.append(icono);

            // 2.Etiqueta de habilidad
            let etiqueta = document.createElement('span');
            etiqueta.classList.add('skill-name');
            etiqueta.innerHTML = habilidad.habilidad;
            skillBox.append(etiqueta);
            ...
            nivel.style.width = `${habilidad.nivel / 4 * 100}%`; 
            barra.append(nivel);
            skillBox.append(barra);

            habilidades.append(skillBox); 
